...
@testset "Shared fixed activity sums match endpoint enumeration" begin
...
@testset "Shared fixed sums preserve stored BigFloat low bits" begin
...
@testset "Shared fixed sums preserve large rational reduction" begin
    T=Rational{BigInt}
...
@testset "A shared prefix does not permit sharing unequal endpoint products" begin
    for T in (Float32,Float64,BigFloat,Rational{BigInt}), pivot in (-2,2), coefficient in (-5,5), side in (:lower,:upper), fixed in (false,true)
        vertices=T[(11-coefficient*z)//pivot for z in (1,2)]
        if fixed
            low=vertices[1]-T(1)/4;high=vertices[1]+T(1)/4
        else
            low=minimum(vertices)+(side==:lower ? T(1)/4 : zero(T))
            high=maximum(vertices)-(side==:upper ? T(1)/4 : zero(T))
        end
        problem=LinearProblem(sparse(reshape(T[pivot,3,coefficient],1,3)),zeros(T,3);
            row_lower=T[14],row_upper=T[14],column_lower=T[low,1,1],column_upper=T[high,1,fixed ? 1 : 2])
        original=deepcopy(problem)
        @test JSimplex._equality_implies_column_bounds(problem,JSimplex._row_entries(problem.A)[1],1,big(pivot)//big(1),big(14)//big(1))==fixed
        @test isequal(problem.A.nzval,original.A.nzval) && isequal(problem.column_lower,original.column_lower) && isequal(problem.column_upper,original.column_upper)
    end
end

@testset "Sparse aggregation reuses matching fixed activity sums" begin
function aggregation_implied_shared_fixed_sums_probe(kind; count=128, T=Float64)
    ...
    for T in (Float32,Float64,BigFloat,Rational{BigInt}), kind in (:integer_positive,:integer_negative,:fraction_positive,:fraction_negative,:variable,:free_pivot)
        count=4;problem,pass=aggregation_implied_shared_fixed_sums_probe(kind;count,T)
    ...
    for (kind,limit) in ((:integer_positive,62915),(:integer_negative,62915),(:fraction_positive,63427),(:fraction_negative,63427),(:variable,66707),(:free_pivot,44365))
