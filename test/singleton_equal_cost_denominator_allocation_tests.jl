...
@testset "Singleton: Equal cost denominators preserve aggregation results" begin
...
@testset "Singleton: Equal-denominator cost differences use committed updates" begin
...
@testset "Singleton: Equal cost denominators retain stored precision and residuals" begin
...
@testset "Singleton: Cost differences canonicalize large non-dyadic fractions" begin
    T=Rational{BigInt}
    for den in (5,7,15,21), factor in (-2,-1,1,2,4)
        ratio=T((BigInt(1)<<300)+1,BigInt(den));old=factor*ratio
        problem=LinearProblem(sparse(T[2 1;0 1]),T[2ratio,old];objective_constant=T(7),
...
@testset "Singleton: Cost denominator shortcut preserves representability rejection" begin
...
@testset "Singleton aggregation avoids general equal-denominator cost subtraction" begin
function singleton_equal_cost_denominator_probe(kind; count=256, T=Float64)
    ...
    for T in (Float32,Float64,BigFloat,Rational{BigInt}), kind in (:integer_positive,:integer_negative,:fraction_positive,:fraction_negative,:unequal_denominator,:zero_ratio), count in (3,4)
        problem,pass=singleton_equal_cost_denominator_probe(kind;count,T)
        original=deepcopy(problem);result=pass(problem)
        ratios=problem.objective[1:count]./2;term=problem.A[1,count+1]
        ...
        expected=problem.objective[end]-term*sum(ratios)
    ...
    for (kind,limit) in ((:integer_positive,53387),(:integer_negative,53387),(:fraction_positive,53451),(:fraction_negative,53451),(:unequal_denominator,53637),(:zero_ratio,40069))
