...
@testset "Implied equal-denominator quotients match endpoint enumeration" begin
    ...
    cases=((3,1,1,1,0),(3,1//2,3//2,1,0),(-3,1//2,3//2,1,0),
           ...
           (3,1//2,1,1,0))
    for T in (Float32,Float64,BigFloat,Rational{BigInt}), pivot in (-3//2,3//2), rhs in (-5,5//2,1//4), (a,lo,hi,b,z) in cases, xb in ((-3,3),(0,0),(nothing,2),(-2,nothing),(nothing,nothing))
...
@testset "Shared quotient denominators preserve stored BigFloat values" begin
    for direction in (-1,1), offset in (-1,0,1), ambient in (32,64,256)
        problem=setprecision(BigFloat,256) do
            tiny=BigFloat(2)^(-200);pivot=direction*(BigFloat(3)+tiny)
            rhs=BigFloat(3)+pivot+offset*tiny
            LinearProblem(sparse(reshape(BigFloat[pivot,3],1,2)),zeros(BigFloat,2);
                row_lower=[rhs],row_upper=[rhs],column_lower=BigFloat[1,1],column_upper=BigFloat[1,1])
        end
        ...
        setprecision(BigFloat,ambient) do
            @test JSimplex._equality_implies_column_bounds(problem,JSimplex._row_entries(problem.A)[1],1,pivot,rhs)==(offset==0)
...
@testset "Shared quotient denominators normalize large signed pivots" begin
    T=Rational{BigInt}
    for den in (5,7,15,21), direction in (-1,1), offset in (0,1)
        pivot=T(direction*((BigInt(1)<<300)*den+1),BigInt(den));rhs=T(3)+2pivot
        problem=LinearProblem(sparse(reshape(T[pivot,3],1,2)),zeros(T,2);
            row_lower=T[rhs],row_upper=T[rhs],column_lower=T[2+offset,1],column_upper=T[2+offset,1])
        original=deepcopy(problem)
        @test JSimplex._equality_implies_column_bounds(problem,JSimplex._row_entries(problem.A)[1],1,pivot,rhs)==(offset==0)
...
@testset "Sparse aggregation divides shared-denominator numerators" begin
function aggregation_implied_equal_quotient_denominator_probe(kind; count=128, T=Float64)
    ...
    for T in (Float32,Float64,BigFloat,Rational{BigInt}), kind in (:integer_positive,:integer_negative,:fraction_positive,:fraction_negative,:unequal,:free_pivot)
        count=4;problem,pass=aggregation_implied_equal_quotient_denominator_probe(kind;count,T)
    ...
    for (kind,limit) in ((:integer_positive,44483),(:integer_negative,44483),(:fraction_positive,45507),(:fraction_negative,45507),(:unequal,47793),(:free_pivot,29873))
